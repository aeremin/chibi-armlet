...
void LSM303::writeAccReg(regAddr reg, uint8_t value) {
    ...
    Buf[0] = (uint8_t)reg;
    Buf[1] = value;
    last_status = i2c.CmdWriteWrite(acc_address, Buf, 2, nullptr, 0);
...
uint8_t LSM303::readAccReg(regAddr reg) {
    ...
    last_status = i2c.CmdWriteRead(acc_address, &r, 1, &value, 1);
...
void LSM303::writeMagReg(regAddr reg, uint8_t value) {
    ...
    Buf[0] = (uint8_t)reg;
    Buf[1] = value;
    last_status = i2c.CmdWriteWrite(mag_address, Buf, 2, nullptr, 0);
...
uint8_t LSM303::readMagReg(regAddr reg) {
    ...
    uint8_t value=0;
    uint8_t r = (uint8_t)reg;
    last_status = i2c.CmdWriteRead(mag_address, &r, 1, &value, 1);
...
void LSM303::readMag(void) {
    ...
    last_status = i2c.CmdWriteRead(mag_address, &reg, 1, (uint8_t*)&m, 6);
    if(_device != device_D) {
...
int LSM303::testReg(uint8_t address, regAddr reg) {
    uint8_t value=0;
    uint8_t r = (uint8_t)reg;
    last_status = i2c.CmdWriteRead(address, &r, 1, &value, 1);
