...
DbgUart_t Uart;
static uint8_t UartBuf[198];

void DbgUart_t::Printf(const char *format, ...) {
    va_list args;
    va_start(args, format);
    uint32_t Cnt = tiny_vsprintf((char*)UartBuf, format, args);
    va_end(args);
    if(Cnt > UART_TXBUF_SIZE) Cnt = UART_TXBUF_SIZE;    // Shrink too long string

    if(IDmaIsIdle) {
        MemCopy(TXBuf, UartBuf, Cnt);// Place string to buffer from beginning
        ...
        if(Cnt < BytesFree) {   // Message fits in buffer, no splitting needed
            MemCopy(PWrite, UartBuf, Cnt);
            PWrite += Cnt;
        }
        else { // Cnt >= BytesFree
            MemCopy(PWrite, UartBuf, BytesFree);
            uint32_t Remainder = Cnt - BytesFree;
            if(Remainder) MemCopy(TXBuf, &UartBuf[BytesFree], Remainder);
